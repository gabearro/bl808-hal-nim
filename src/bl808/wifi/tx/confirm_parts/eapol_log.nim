proc logEapolTxConfirm(ethHdr: ptr EthernetHeaderView; txhdr: pointer; cb: TxCallback;
                       ethTypeBytes: uint16; txConfirmResult: int; txStatusWord: uint32) =
  if ethTypeBytes != 0x8e88'u16 and ethTypeBytes != 0x888e'u16:
    return

  {.emit: ["{ extern volatile unsigned int nimfw_dbg_cfm_cb_ptr_last; nimfw_dbg_cfm_cb_ptr_last = (unsigned int)", cb, "; extern volatile unsigned int nimfw_dbg_cfm_last_ethertype; nimfw_dbg_cfm_last_ethertype = (unsigned int)", ethTypeBytes, "; extern volatile unsigned int nimfw_dbg_eapol_cfm_status; nimfw_dbg_eapol_cfm_status = (unsigned int)", txStatusWord, "; extern volatile unsigned int nimfw_dbg_eapol_cfm_count; nimfw_dbg_eapol_cfm_count++; }"].}
  ...
  nimFwDbgEapolCfmStatusLog[logIdx] = txStatusWord
  nimFwDbgEapolCfmMetaLog[logIdx] =
    eapolLen.uint32 or
    (if txConfirmResult > 0: 1'u32 shl 16 else: 0'u32) or
