...
when defined(nimNoLibc) or defined(bl808kernel):
  ...
  type
    TaskGroupErrorPolicy* = enum
      epCollectAll
    AsyncStream* = ref object of RootObj
    BufferedReader* = ref object of RootObj
    TcpListener* = ref object of RootObj
    TaskGroup* = ref object of RootObj

  proc newTaskGroup*(policy: TaskGroupErrorPolicy): TaskGroup =
    discard policy
    TaskGroup()
else:
