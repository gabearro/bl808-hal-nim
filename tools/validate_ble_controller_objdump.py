...
* Are the converted BLE RF/coexistence ABI symbols present as real Nim exports?
"""
...
def build_report(ref: Path, nim: Path, source: Path) -> dict[str, object]:
    ...
    ble_rf_symbols: list[dict[str, object]] = []
    for symbol in BLE_RF_SYMBOLS:
        nim_instr = function_instruction_count(nim_dis, symbol)
        ble_rf_symbols.append(
            {
                "symbol": symbol,
                "present": symbol in nim_funcs,
                "nim_instr": nim_instr,
                "line": line_map.get(symbol),
                "placeholder_stub": symbol in stub_symbols,
                "likely_active_placeholder_stub": likely_active_placeholder_stub(
                    symbol,
                    nim_instr,
                    line_map,
                    stub_lines,
    ...
    return {
        ...
        "ble_rf_symbols": ble_rf_symbols,
        "missing_ble_rf_symbols": [
            item["symbol"] for item in ble_rf_symbols if not item["present"]
        ],
        "active_ble_rf_placeholder_stubs": [
            item["symbol"]
            for item in ble_rf_symbols
            if item["likely_active_placeholder_stub"]
        ],
    }
...
def write_markdown(report: dict[str, object], path: Path) -> None:
    ...
    ble_rf_symbols = report["ble_rf_symbols"]
    assert isinstance(missing, list)
    assert isinstance(smaller, list)
    assert isinstance(stubs, list)
    assert isinstance(buckets, dict)
    assert isinstance(ble_rf_symbols, list)
    ...
    lines += [
        "## BLE RF / Coexistence Symbols",
        "",
        "| Symbol | present | src line | nim instr | source stub | active stub |",
        "|---|---|---:|---:|---|---|",
    ]
    for item in ble_rf_symbols:
        ...
        line = item.get("line")
        lines.append(
            f"| `{item['symbol']}` | {'yes' if item['present'] else 'no'} | "
            f"{line if line is not None else ''} | {item['nim_instr']} | "
            f"{'yes' if item['placeholder_stub'] else 'no'} | "
...
def main() -> int:
    ...
    parser.add_argument(
        "--fail-on-missing-ble-rf-symbols",
        action="store_true",
        help=(
            "fail if converted BLE RF/coex ABI symbols are absent from the "
            "Nim object or still generated by an active placeholder stub"
        ),
    )
    ...
    print(
        "Missing BLE RF/coex symbols: "
        f"{len(report['missing_ble_rf_symbols'])}"
    )
    print(
        "Active BLE RF/coex placeholder stubs: "
        f"{len(report['active_ble_rf_placeholder_stubs'])}"
    )
    ...
    if args.fail_on_missing and report["missing_in_nim"]:
        return 1
    if args.fail_on_missing_ble_rf_symbols and (
        report["missing_ble_rf_symbols"]
        or report["active_ble_rf_placeholder_stubs"]
    ):
        return 1
    return 0
